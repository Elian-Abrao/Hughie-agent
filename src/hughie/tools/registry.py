...
from hughie.tools.ssh_tools import SSH_TOOLS
...
async def load_all_tools() -> list:
    ...
    tools = BRAIN_TOOLS + SHELL_TOOLS + FILESYSTEM_TOOLS + SSH_TOOLS + WEB_TOOLS + mcp_tools

    logger.info(
        "Tools loaded: brain=%d shell=%d fs=%d ssh=%d web=%d mcp=%d  total=%d",
        len(BRAIN_TOOLS),
        len(SHELL_TOOLS),
